...
async fn main() {
    ...
    let prev_block_number = BlockId::Number(BlockNumberOrTag::from(1000007));
    let current_block_number = BlockId::Number(BlockNumberOrTag::from(1000008));

    let prev_block = provider
        .get_block(prev_block_number, BlockTransactionsKind::Full)
        .await
        .unwrap()
        .unwrap();
    ...
    let proof_sender = provider
        .get_proof(sender_address, vec![B256::ZERO])
        .block_id(prev_block_number)
        .await
        .unwrap();
    let proof_receiver = provider
        .get_proof(receiver_address, vec![B256::ZERO])
        .block_id(prev_block_number)
        .await
        .unwrap();
    let proof_miner = provider
        .get_proof(miner_address, vec![B256::ZERO])
        .block_id(prev_block_number)
        .await
        .unwrap();
    ...
    state_trie
        .account_trie
        .set_nonce(sender_address, U256::from(sender_new_nonce))
        .unwrap();
    state_trie
        .account_trie
    ...
    assert_eq!(state_trie.root().unwrap(), current_block.header.state_root);
